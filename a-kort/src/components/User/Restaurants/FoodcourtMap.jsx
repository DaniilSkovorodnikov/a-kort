...
export default function FoodcourtMap({positions, setCurrentFoodcourt, setVisible}){
    ...
    useEffect(() => {
        navigator.geolocation.getCurrentPosition((pos) => {
            ...
            const foodcourts = positions.map((v) => getDistance(currentPos.userLatitude, currentPos.userLongitude, v.latitude, v.longitude));
                ...
                {positions.map((v, i) =>
                    <li className="map__item">
                        ...
                        <button className="map__change" onClick={() => {
                            setNearest(positions.find((pos) => pos.name === v.name))
                        }}>Выбрать</button>
                    </li>
                )}
            </ul>
